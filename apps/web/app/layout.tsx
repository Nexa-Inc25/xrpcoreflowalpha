    ...
    <html lang="en" className="antialiased">
      <head>
        <Script id="zk-ws-shim" strategy="beforeInteractive">
          {`(function(){try{if(typeof window==='undefined')return;var WS=window.WebSocket;if(typeof WS!=='function')return;if(WS.__zkPatched)return;var base=${JSON.stringify(process.env.NEXT_PUBLIC_API_WS_BASE || 'wss://api.zkalphaflow.com')};var baseTrim=String(base).replace(/\/$/,'');var Patched=function(url,protocols){var raw=(typeof url==='string')?url:(url&&url.toString?url.toString():String(url));var next=raw;try{if(raw.includes('localhost:8010')||raw.includes('127.0.0.1:8010')||raw.startsWith('ws://localhost')||raw.startsWith('ws://127.0.0.1')){next=baseTrim+'/api/events';console.log('[WS Shim] Redirected:',raw,'to:',next);}}catch(e){console.error('[WS Shim] Error:',e);}return protocols!=null?new WS(next,protocols):new WS(next);};Patched.prototype=WS.prototype;Patched.__zkPatched=true;window.WebSocket=Patched;}catch(e){}})();`}
        </Script>
